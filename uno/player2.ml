...
(** [color_search_bounds t1 hand color lwr upr] is the name of the first card in 
...
let rec color_search_bounds t1 hand color lwr upr =
  ...
  | h::t -> if (Gamestate.color_search t1 Player h = color 
                || Gamestate.color_search t1 Player h = "black")
...
let rec color_count t1 hand color counter =
  ...
  | [] -> counter
  | h::t -> if Gamestate.color_search t1 Player h = color 
    then color_count t1 t color counter+1 
...
(** [wildcard_color t hand] is the color that player will choose when playing a 
...
let choose_color t =
  wildcard_color t (Gamestate.hand t Player)